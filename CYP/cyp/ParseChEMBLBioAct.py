...
def ChEMBL_Data_cleanup(FileName):
	...
	with files as fd:
		...
		for row in rd:
			...
			if ChEMBL_ID not in ChEMBLID_Tracker:
				...
				if activity != "":
					if "substrate" in activity.lower() and "not" not in activity:
					...
					elif "inhibitor" in activity.lower() and "not" not in activity:
	...
	print("substrate_v_inhibitor: " + str(len(substrate_v_inhibitor)))
	active_v_inactive = Active + inactive
	print("active_v_inactive: " + str(len(active_v_inactive)))
	...
	new_chembl_file = open(file_name,"r")
	...
	for duplicate_compound in duplicated:
		...
		tmp_id_list = [str(chembl_id)]
		writer_csv_4.writerow(tmp_id_list)
